...
public class FoodHomeAdapterDinner extends RecyclerView.Adapter<FoodHomeAdapterDinner.FoodHomeDinnerViewHolder>{
    ...
    public FoodHomeAdapterDinner(List<Food> listFood, Context context, SharedPreferences sharedPreferences) {
    ...
    @NonNull
    @Override
    public FoodHomeAdapterDinner.FoodHomeDinnerViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_list_home, parent, false);
        return new FoodHomeAdapterDinner.FoodHomeDinnerViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull FoodHomeAdapterDinner.FoodHomeDinnerViewHolder holder, int position) {
        ...
        holder.imgDelete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                deleteFoodIdToListFoodDinnerToday(f);
                context.startActivity(new Intent(context, context.getClass()));
            }
        });
    }
    private void deleteFoodIdToListFoodDinnerToday(Food foodDetail) {
        String listfavo = sharedPreferences.getString("LIST_FOOD_DINNER_TODAY"+ email, "");
        ...
        if(listfavo != null){
            if(listfavo.trim().length() > 0){   //cắt khoản trống ' ' đầu list
                ...
                editor.putString("LIST_FOOD_DINNER_TODAY"+email, listfavo);
                editor.commit();
                Toast.makeText(context, "delete "+foodDetail.getFoodName()+" from dinner success", Toast.LENGTH_SHORT).show();
    ...
    public class FoodHomeDinnerViewHolder extends RecyclerView.ViewHolder{
        private TextView NameElement, InfoNameElement;
        private CardView cardview_Ele;
        private ImageView imgDelete;
        public FoodHomeDinnerViewHolder(@NonNull View itemView) {
            super(itemView);
            //-----------item_list_home
